...
| 待做 | netd Phase 2+：扩协议（bind/listen/accept、UDP、DNS-via-poll、并发/异步）、**共享 DS 大数据**（绕 inline IPC ~250B 上限——ifconfig/ping 多接口/多包会撞）、透明 libc socket 后端（`libc_be_socket_netd`）、迁移 udp/net/mqtt/telnetd（udp 需 UDP、telnetd 需 listen/accept、mqtt 需重写 socket 层+TLS）到 net-cluster/tools、终态彻底从 native_shell 摘掉 lwIP |
