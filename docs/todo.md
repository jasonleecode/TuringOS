...
| [✓] | **mqtt 迁为独立工具（plaintext pub/sub，2026-06-12）**：新 `pkg/net-cluster/tools/mqtt`，**工具内自实现 MQTT 3.1.1 框架**（CONNECT/CONNACK/PUBLISH/SUBSCRIBE/SUBACK/PINGREQ/DISCONNECT，varint remaining-length 编解码）走 netd TCP（tcp_connect/send/recv/close）——协议逻辑在工具，netd 只搬字节，不再依赖 lwIP mqtt app。为让 `sub` 轮询不卡死 netd：netd 给 tcp_connect socket 加 **SO_RCVTIMEO 2s**，op_recv 把超时映射成空结果（length 0），工具循环轮询 N 秒。native_shell 删 mqtt builtin。`run rom/mqtt pub|sub <broker> <topic> [msg|secs]`。验证 `tools/mqtt_verify.py`（Python MQTT broker stub，pub→broker 收到 hello_mqtt、sub→收到 broker 推送 from_broker）PASS；ci_smp_smoke 3/3。**TLS(pubs/subs/:8883) 暂不支持**——需 netd 侧 TLS（后续） |
| 待做 | **telnetd 迁移（剩最后 1 个 net 命令，迁完才能摘 lwIP）**：telnetd＝远程 shell，需 netd accept + 把 socket I/O 接到 spawn 的 native_shell（console-over-IPC 重定向，难——原版进程内 dup2 socket→stdout 复用命令分发）。独立子工程 |
| 待做 | mqtt TLS（mqtts/:8883）：需 netd 侧 TLS（tcp_connect_tls，复用 pkg/mbedtls）或工具内 mbedTLS-over-netd transport |
| [✓] | **native_shell 彻底去网络化（2026-06-12，头号目标达成）**：所有 net 命令都已迁出（builtin 全删）→ 删进程内 net 栈：`cmd_net.cc`(933,virtio-net 驱动+lwIP+DHCP)/`cmd_ping.cc`(333)/`cmd_telnet.cc`(60)/死的 `log.cc`(194)，从 Makefile REQUIRES+Control 摘 `lwip/libc_be_socket_lwip/libsigma0/nc_mqtt/nc_telnet`，删 `net_auto_init/net_is_ready`+boot 等待。**native_shell 现零网络代码、零 sigma0(裸物理内存)cap**，4513→2964 行（砍 ~1550 行+整个 TCP/IP 栈）。验证：登录(authd)+netcat(netd) 正常；ci_smp_smoke 3/3。commit 36eed19 |
| 待做(WIP) | **telnetd 工具**（远程 shell over netd vcon 桥）：已写好+能编译，桥跑到 spawn_con 调用，但**卡 `-2008` cap 传递 IPC**（spawnd 收不到 vcon cap）。改动在工作树未提交（telnetd 工具/cfg spawnd_caps/modules.list）。需 debug spawnd cap-receive（Br_manager cap demand / 声明细节） |
| 待做 | netd Phase 2 续：socket-over-IPC 的 UDP、DNS-via-poll、**共享 DS 大数据**（绕 inline IPC ~250B 上限）、透明 libc socket 后端（`libc_be_socket_netd`）、mqtt TLS(mqtts) |
