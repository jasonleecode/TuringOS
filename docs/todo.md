...
| 待做 | netd Phase 2+：扩协议（bind/listen/accept、UDP、DNS、poll、共享 DS 大数据、并发/异步）、透明 libc socket 后端（`libc_be_socket_netd`）、迁移其余 net 命令（dhcp/nslookup 简单；ping 需 raw ICMP、udp 需 UDP、telnetd 需 listen/accept）到 net-cluster/tools、终态彻底从 native_shell 摘掉 lwIP；③ 续：telnetd/mqtt 等真 app 迁为独立进程 |
